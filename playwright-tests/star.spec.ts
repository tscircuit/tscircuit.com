...
test("test for starring a repo", async ({ page }) => {
  ...
  const addStarPromise = page.waitForResponse(resp => resp.url().includes('/api/snippets/add_star'))
...
test("test for removing a star from a repo", async ({ page }) => {
  ...
  const removeStarPromise = page.waitForResponse(resp => resp.url().includes('/api/snippets/remove_star'))
