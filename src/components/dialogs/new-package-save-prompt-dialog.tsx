...
export const NewPackageSavePromptDialog = ({
  ...
  useEffect(() => {
    if (organizations && organizations.length > 0 && !selectedOrgId) {
      setSelectedOrgId(organizations.find(x=>x.is_personal_org)?.org_id || organizations[0].org_id)
    }
            ...
            <Select value={selectedOrgId} onValueChange={setSelectedOrgId} disabled={orgsLoading}>
                ...
                <div className="flex items-center gap-2 flex-1">
                  {selectedOrgId && organizations ? (
                    <span className="truncate">
                      {organizations.find(org => org.org_id === selectedOrgId)?.display_name || 
                       organizations.find(org => org.org_id === selectedOrgId)?.name || 
              ...
              <SelectContent className="!z-[999]">
                {organizations?.length === 0 ? (
                  <div className="px-2 py-1.5 text-sm text-slate-500">No organizations found</div>
                ) : (
                  organizations?.map((org) => (
                    <SelectItem key={org.org_id} value={org.org_id} className="cursor-pointer">
                      {org.display_name || org.name || `Org ${org.org_id.slice(0, 8)}`}
                    </SelectItem>
                  ))
                )}
              </SelectContent>
