...
export const SingleBuildLogs = ({
  ...
  const logStreamUrl = packageBuild?.user_code_job_log_stream_url

  const shouldStreamLogs = userCodeJobInProgress

  const sseKey = `${packageBuild?.package_build_id || "none"}-${logStreamUrl || "none"}`
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-600 max-w-md">
                This build could not be found or may have been deleted.
              </p>
            </div>
          </div>
        </div>
