...
export const getLicenseFromLicenseContent = (licenseContent: string): LicenseType => {
  // Normalize the license content by removing extra whitespace and converting to lowercase
  const normalizedContent = licenseContent.toLowerCase().replace(/\s+/g, ' ').trim()

  let bestMatch: { type: LicenseType; ratio: number } = { type: 'unset', ratio: 0 }
