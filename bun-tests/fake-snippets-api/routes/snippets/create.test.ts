...
test("create snippet and call package get to verify it exists", async () => {
  ...
  expect(packageResponse.data.package.unscoped_name).toBe("TestSnippet")
  ...
  const packageFileResponse = await axios.post(
    `/api/package_files/get`,
    {
  ...
  expect(packageFileResponse.data.package_file.content_text).toBe("Test Content")
})
